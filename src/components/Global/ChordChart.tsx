...
const ChordChart: FunctionComponent<Props> = (props) => {
  ...
  let tuningContainerHeight = 30
  let chartWidth = width * 0.85
  ...
  let circleRadius = chartWidth / 18
  let bridgeStrokeWidth = Math.ceil(chartHeight / 36)
  let fontSize = Math.ceil(chartWidth / 9)
  let fontSizeFret = Math.ceil(chartWidth / 12)
  ...
  let fretLabelTextWidth = width / 13
  //start draw func
  ...
  function drawFret(x: number, y: number, msg: string) {
    return <Text
      key={"text-" + x + y + msg}
      fill={defaultColor}
      stroke={defaultColor}
      fontSize={fontSizeFret}
      x={x}
      y={y}
      textAnchor="middle"
    >
